...
async fn chat_completions(
    ...
    if is_stream {
        ...
        if log_response_body {
            // Shadow stream mode: tee chunks to both client and background logger
            let (shadow_tx, shadow_rx) = mpsc::unbounded_channel::<ByteChunk>();

            let raw_stream = upstream_resp.bytes_stream();

            // Create a stream that forwards chunks to client while also sending copies to shadow_tx
            ...
            // Spawn background task to accumulate shadow chunks and log
            ...
            tokio::spawn(async move {
                ...
                // Parse SSE buffer to extract usage from the last data event
            ...
            // No response body logging: passthrough stream, log metadata only
...
/// Parse concatenated SSE bytes to extract `usage` from the last `data:` event.
...
fn parse_sse_usage_and_body(
    ...
    // Collect all `data:` lines that aren't `[DONE]`
    let mut last_data_json: Option<serde_json::Value> = None;
    ...
    for line in text.lines() {
        ...
        if let Some(data) = line.strip_prefix("data:") {
            ...
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(data) {
                all_chunks.push(json.clone());
                last_data_json = Some(json);
            }
        }
    }

    // Extract usage — many providers put it in the last chunk
    let (prompt_tokens, completion_tokens, total_tokens) = last_data_json
        .as_ref()
        .and_then(|j| j.get("usage"))
        .map(|usage| {
            (
    ...
    (prompt_tokens, completion_tokens, total_tokens, response_body)
}
